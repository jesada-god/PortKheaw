...
### Q5 · จำนวน alert ต่อแถว — **ปิดแล้ว 2026-09-20: เลือก (ค)**
ตอบว่า **นับจาก `price_alerts`** และไปไกลกว่านั้น: `202609200001` รวมสองระบบเป็นระบบเดียว
`overview_alert_rules` / `overview_alert_hits` ถูก drop ทิ้ง ย้าย kind `earnings` —
ความสามารถเดียวที่ของใหม่มีจริง — เข้ามาเป็นเงื่อนไขที่ห้าของ `price_alerts`

ข้อที่เคยเป็นข้อแม้ของ (ค) — "เป็นคนละระบบกับที่ §5.5 ของ CONTRACT แยกไว้ตั้งใจ" — หมดไป
เพราะการแยกนั้นหมดอายุตั้งแต่ `202608310001` ให้ scheduler กับ cooldown กับตารางเขียนผล
แก่ของใหม่ ทำให้มันกลายเป็นระบบเดียวกับของเดิม §5.5 ถูกทำเครื่องหมายยกเลิกไว้แล้ว

badge นับจากแถวที่ `app/page.tsx` อ่านอยู่แล้วสำหรับ Upcoming feed จึง **ไม่เสีย query เพิ่มเลย**
และแยก `{}` (ไม่มี alert) ออกจาก `null` (อ่านไม่ได้) ไว้ตามเดิม
