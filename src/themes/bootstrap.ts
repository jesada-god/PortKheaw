import { PREMIUM_THEMES } from './types';

/**
 * Runs synchronously in <head>. Keep the emitted script dependency-free: it must
 * restore the presentation preference before CSS paints, without evaluating
 * application stores or touching unrelated persisted settings. The one import
 * below is inlined into the string at build time, so nothing is loaded at
 * runtime — and the gated list cannot drift from the gate it mirrors.
 *
