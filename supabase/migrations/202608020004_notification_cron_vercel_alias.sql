...
begin
  if input_url <> 'https://portkheaw.vercel.app/api/cron/alerts' then
  ...
  select cron.schedule(
    ...
    $command$
      select net.http_get(
        url := 'https://portkheaw.vercel.app/api/cron/alerts',
...
begin
  if not exists (
    ...
    raise exception 'Notification cron secret is not configured' using errcode = 'P0002';
  end if;
